...
function displayOrderItems() {
  ...
  if (cart.length === 0) {
    orderItemsDiv.innerHTML = '<p style="text-align: center; color: #999;">Your cart is empty</p>';
    return;
  }
...
function placeOrder() {
  ...
  fetch(apiUrl, {
    ...
    .then(data => {
      clearCart();
      alert("✅ Order placed successfully!\nOrder ID: " + (data.orderId || "Your order has been received"));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const totals = getCartTotals();
  document.getElementById("subtotal").textContent = totals.totalPrice.toLocaleString();
  document.getElementById("totalAmount").textContent = totals.totalPrice.toLocaleString();
